function [bnf,blm] = md_waterShed(bw,bm)

% Input:
%    -bw the binary image
%    -bm the binary marker image
% Output:
%    -bnf the segmented nuclei regions
%    -blm: the segmented nuclei boundary contours
...
%% distance transform
CC = bwconncomp(bw);
LL = labelmatrix(CC);                                                      %% assume background is labelled as 0
[m,n] = size(bw);
bw_dist = zeros(m,n);
bm_temp=false(m,n);
bw_temp=false(m,n);
...
%indm=find(L==ind | L==0);
bn(L==ind | L==0)=0;
bnf=bn|(bw-bw_temp);                                                       %% segmented nuclei regions

blm=bwperim(bw-bw_temp);
%ind3=find(L==0);
blm(L==0)=1;                                                               %% segmented nuclei contours
end
