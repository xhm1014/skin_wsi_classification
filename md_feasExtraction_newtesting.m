...
Outputpath3='C:\Users\mxu\Desktop\ClassfInterRes\Nov-2017\features\'; %% for saving intermediate results
AnalysisMagnification=27.2;                                    %% image magnification
ResAt40X=0.25;                                          %% micrometer/per pixel
ResAtcurAnaMag=ResAt40X*(40/AnalysisMagnification);
Foldername='C:\Users\mxu\Desktop\skin\testing\';              %% the path to store new testing images

index=1;
%% 1. process WSIs
for idx4Set=1:15
    if idx4Set<10
        imageName=strcat('t0',num2str(idx4Set),'.tif');
    else
        imageName=strcat('t',num2str(idx4Set),'.tif');
    end
    
    disp(sprintf('Processing the %dth biospy \n',idx4Set));
    wholeFilename=[Foldername imageName];
...
%end
outfilename=sprintf('%s%s.mat',Outputpath3,'non-melanoma');
