...
%*** Check input ****
if nargin < 2                      %number of function arguments
    error('no bandwidth specified')
end

if nargin < 3
%    plotFlag = true;     % to visualize the iterations
    plotFlag = false;
end
...
while numInitPts
    ...
    while 1     %loop untill convergence
        ...
        sqDistToAll = sum((repmat(myMean,1,numPts) - dataPts).^2);    %dist squared from mean to all points still active sum each column
        inInds      = find(sqDistToAll < bandSq);               %points within bandWidth
        thisClusterVotes(inInds) = thisClusterVotes(inInds)+1;  %add a vote for all the in points belonging to this cluster
        
        
        myOldMean   = myMean;                                   %save the old mean
        myMean      = mean(dataPts(:,inInds),2);                %compute the new mean  the mean value of each row
