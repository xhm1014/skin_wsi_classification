...
function AllSP=LfindOutterSPV3(Centroid,Area,im,bw,GC_x,GC_y,MeanIntensity,Regularization,~)
%% radial orientations
RlineNO=50;
...
radi_max =ceil(2.6*sqrt(Area/pi)); % max radius
...
%        alpha360_m=LcalAccAngle4Direction(curPtsonLine,GC_x,GC_y);
        ...
        alpha_m=-alpha_m;
        alpha360_m=alpha_m;
        alpha360_m(alpha_m<0)=pi+alpha_m(alpha_m<0)+pi;
    ...
    for i=1:length(theta)
        ...
        if abs(curdist-curdist_s)>TdiffbetweenSmoothandOri
            ...
            if ~isempty(curPtsonLine)
                ...
                [~,minind]=min(abs(curdist_s-distance2Ct));
